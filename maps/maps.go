...
// Keys returns a comb.Comb that extracts the keys of a map and returns them as
// a slice.
...
// Values returns a comb.Comb that extracts the values of a map and returns
// them as a slice.
...
// SortedKeys returns a comb.Comb that extracts the keys of a map and returns
// them as a sorted slice.
func SortedKeys[K cmp.Ordered, V any]() comb.Comb[map[K]V, []K] {
	return Keys[K, V]().Then(slices.Sort[K]())
}

// SortedKeysFunc returns a comb.Comb that extracts the keys of a map and
...
// SortedValues returns a comb.Comb that extracts the values of a map and
// returns them as a sorted slice.
func SortedValues[K comparable, V cmp.Ordered]() comb.Comb[map[K]V, []V] {
	return Values[K, V]().Then(slices.Sort[V]())
}

// SortedValuesFunc returns a comb.Comb that extracts the values of a map and
// returns them as a sorted slice using a custom comparison function.
